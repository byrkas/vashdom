...
use Application\Entity\Admin as User;
...
class UserManager
{
    ...
    public function addUser($data)
    {
        ...
        if($this->checkUserExists($data['email'])) {
            $this->flashMessenger->addErrorMessage("User with email address " . $data['$email'] . " already exists");
    ...
    public function generatePasswordResetToken($user)
    {
        ...
        $passwordResetUrl = 'http://' . $httpHost . '/backend/reset-password?token=' . $token;
    ...
    {
        $userEntity = $this->entityManager->getReference('Application\Entity\Admin',$user->getId());
        ...
        if (strlen($newPassword)<6 || strlen($newPassword)>64) {
            $this->flashMessenger->addErrorMessage('New password is very week\strong!');
            return false;
        }
        ...
        $userEntity->setPasswordResetTokenCreationDate(null);

        $this->entityManager->flush();
        $this->flashMessenger->addSuccessMessage('Password was changed!');
